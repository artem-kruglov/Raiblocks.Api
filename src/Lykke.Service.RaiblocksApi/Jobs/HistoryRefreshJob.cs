...
namespace Lykke.Service.RaiblocksApi.Jobs
{
    public class HistoryRefreshJob
    {
        ...
        public HistoryRefreshJob(ILog log, IBlockchainService blockchainService,
        ...
        private async Task RefreshHistory(AddressObservationType type)
        {
            await _log.WriteInfoAsync(nameof(HistoryRefreshJob), $"Env: {Program.EnvInfo}",
                $"History job {Enum.GetName(typeof(AddressObservationType), type)} start", DateTime.Now);
            ...
            do
            {
                addressObservations = await _historyService.GetAddressObservationAsync(pageSize, continuation,
                    Enum.GetName(typeof(AddressObservationType), type));
                ...
                foreach (var addressObservation in addressObservations.items)
                {
                    ...
                    {
                        var currentHistoryTo = (await _historyService.GetAddressHistoryAsync(addressBlockCount,
                            Enum.GetName(typeof(AddressObservationType), AddressObservationType.To),
                            addressObservation.Address)).items;

                        var currentHistoryFrom = (await _historyService.GetAddressHistoryAsync(addressBlockCount,
                            Enum.GetName(typeof(AddressObservationType), AddressObservationType.From),
                            addressObservation.Address)).items;
                        ...
                        {
                            var history = await _blockchainService.GetAddressHistoryAsync(addressObservation.Address,
                                addressBlockCount - latestBlockNum);
                            ...
                            var addressHistoryEntries = history.Select((x, index) => new AddressHistoryEntry
                            {
                                ...
                                Hash = x.hash,
                                Type = x.type == TransactionType.send
                            ...
                            await _log.WriteInfoAsync(nameof(HistoryRefreshJob),
                                JArray.FromObject(addressHistoryEntries).ToString(),
                                $"History {Enum.GetName(typeof(AddressObservationType), type)}  {addressObservation.Address} sync");
                        }
                        else if (addressBlockCount == latestBlockNum)
                        {
                            await _log.WriteInfoAsync(nameof(HistoryRefreshJob), $"Env: {Program.EnvInfo}",
                                $"All history {Enum.GetName(typeof(AddressObservationType), type)} {addressObservation.Address} is already sync ");
                        }
                        else
                        {
                            await _log.WriteErrorAsync(nameof(HistoryRefreshJob), $"Env: {Program.EnvInfo}",
                                $"Block count less then db records. Address: {addressObservation.Address}", null);
                        }
                    }
                }
